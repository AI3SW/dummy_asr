...
class DummyASRWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def __init__(self, application, request, **kwargs):
        ...
        self.utt_id = 0
        self.transmission_thread = threading.Thread(target=self._run)
    ...
    def on_message(self, message):
        if isinstance(message, str):
            ...
            logging.info("[Sock %s] Message received" %
                         (self.sock_index, ))
        ...
        elif self.start_transmission and not self.transmission_thread.is_alive():
            self.transmission_thread = threading.Thread(target=self._run)
